...
{
    if ( isOpen() ) {
        qWarning() << "QSerialPort already open";
        return false;
    }
    ...
    if ( d->device.startsWith( "sim:" ) ) {
        ...
        if ( addr.sin_addr.s_addr == INADDR_NONE ) {
            ...
            if ( !ent ) {
                qDebug() << "could not resolve " << lhost;
                ::close( d->fd );
                d->fd = -1;
                return false;
            }
        ...
        if ( ::connect( d->fd, (struct sockaddr *)&addr, sizeof(addr) ) < 0 ) {
            qDebug() << "could not connect to phone simulator at " << lhost;
            ::close( d->fd );
            d->fd = -1;
            return false;
        }
        ...
        if ( d->fd == -1 ) {
            qDebug() << "QSerialPort " << d->device << " cannot be openned";
            perror("QSerialPort cannot open");
            return false;
        }
        d->isTty = ::isatty( d->fd );

        qDebug() << "Device:" << d->device << "is a tty device:" << (d->isTty ? "True" : "False");
        ...
        qDebug() << "NONBLOCK successfully reset";
    }
#endif
#ifdef USE_TERMIOS
    if ( d->isTty ) {
        ...
        if( ::tcsetattr( d->fd, TCSANOW, &t ) < 0 )
            qDebug() << "tcsetattr(" << d->fd << ") errno = " << errno;
